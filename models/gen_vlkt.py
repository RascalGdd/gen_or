...
class GEN_VLKT(nn.Module):
    def __init__(self, backbone, transformer, num_queries, aux_loss=False, args=None):
        ...
        if args.with_clip_label:
            ...
            self.hoi_class_embedding = nn.Linear(args.clip_embed_dim, 15)
        ...
        if args.with_obj_clip_label:
            ...
            self.sbj_class_embed = nn.Linear(hidden_dim, num_obj_classes + 1)
    ...
    def forward(self, samples: NestedTensor, is_training=True):
        ...
        if self.args.with_obj_clip_label:
            ...
            outputs_sbj_class = self.sbj_class_embed(o_hs)
        ...
        out = {'pred_hoi_logits': outputs_hoi_class[-1], 'pred_sub_logits': outputs_sbj_class[-1],
        ...
        if self.aux_loss:
            ...
            out['aux_outputs'] = self._set_aux_loss_triplet(outputs_hoi_class, outputs_sbj_class, outputs_obj_class,
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_sub_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        idx = self._get_src_permutation_idx(indices)
        target_classes_s = torch.cat([t['sub_labels'][J] for t, (_, J) in zip(targets, indices)])
        target_classes = torch.full(src_logits.shape[:2], self.num_obj_classes,
                                    dtype=torch.int64, device=src_logits.device)
        target_classes[idx] = target_classes_s

        loss_sub_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
    ...
    def get_loss(self, loss, outputs, targets, indices, num, **kwargs):
        if 'pred_hoi_logits' in outputs.keys():
            ...
            loss_map = {
                ...
                'sub_cardinality': self.loss_sub_cardinality,
...
class PostProcessHOITriplet(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        for index in range(len(hoi_scores)):
            hs, os, ss, ol, sb, ob, sl, hoi_l = hoi_scores[index], obj_scores[index], sub_scores[index], obj_labels[index], sub_boxes[index], obj_boxes[
                index], sub_labels[index], hoi_labels[index]
            ...
            results[-1].update({'hoi_scores': hs.to('cpu'), 'obj_scores': os.to('cpu'), 'sub_scores': ss.to('cpu'),
